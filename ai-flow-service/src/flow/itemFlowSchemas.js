...
function normalizeLocalizedNumberInput(value) {
  if (value == null || value === '') {
    return null;
  }
...
const localizedNumber = z
  .preprocess((value) => normalizeLocalizedNumberInput(value), z.coerce.number().nullable());
  ...
  .superRefine((data, ctx) => {
    if (data.isMatch) {
      ...
      if (!data.matchedProductId) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'matchedProductId is required when isMatch is true', path: ['matchedProductId'] });
      }
    }
  });

...
function truncateForLog(value, maxLength = MAX_LOG_STRING_LENGTH) {
...
export async function runExtractionAttempts({
  ...
  while (attempt <= maxAttempts) {
    ...
    if (__searchQueries?.length) {
      ...
      lastSupervision = `ADDITIONAL_SEARCH_REQUESTED: ${__searchQueries.join(' | ')}`;
      lastValidated = null;
    ...
    const validated = { success: true, data: candidateData };
    lastValidationIssues = null;

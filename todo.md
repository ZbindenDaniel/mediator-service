...
0z2. ✅ **Intake reference matching failed on identical names + "HP HP HP" brand triplication.** `/api/intake/start`'s `findRefCandidates` matched the scanned model only against `item_refs."Kurzbeschreibung"`, but imported refs keep the model in `Artikelbeschreibung` (importer maps `notes → Kurzbeschreibung`), so every ERP-imported ref missed — the operator was pushed to create a duplicate. Compounded by the netboot image embedding a (sometimes doubled) vendor inside `model`, which also broke the substring match and produced triple-brand names via the unconditional `[Hersteller, model].join(' ')`. Fixed: matching now searches `Artikelbeschreibung`/`Suchbegriff`/`Kurzbeschreibung` (vendor against `Hersteller` OR description) with the leading vendor stripped from the model term; new `backend/lib/intake-naming.ts` (`stripLeadingVendor`/`composeRefName`) prepends the brand exactly once, used in `findOrCreateRef` and defensively in `intake-complete`. See intake changelog #911. **Deferred:** substring (not fuzzy) matching; no backfill of existing duplicate/triplicated refs; external intake-station TUI pre-fill unchanged.

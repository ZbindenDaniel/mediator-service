...
50. **Rework mechanism (plan then implement).** A way to re-run items when small things change (contract updated, operator wants a text rewrite), without a full re-search. Candidate pieces: durable run-mode column (#49), targeted missing-field enrich (#45 "Gezielt anreichern"), a text-rewrite-only mode (no re-search/categorize/price), and contract-version stamping (#46) to detect items enriched against an outdated contract. Per operator decision this needs its own planning pass once the naming/skip-search/cancel-fail/auto-approve work has landed.

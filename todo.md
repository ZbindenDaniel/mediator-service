...
0z4. **AI runs optimization — phased implementation** (design: `docs/PLANNING_ai_runs_optimization.md`).
  ...
  - **Next — Phase 2 (state-machine correctness):** Thread 4A (separate infra-cancel into a non-terminal/re-queue class instead of `failed`) + Thread 4C (repair an empty `SearchQuery` instead of failing). Then phase 3 (grounding: Thread 1 — category-anchored search + relevance gate), phase 4 (rework closure: Thread 3).

...
50. **Rework mechanism / review-by-exception.** — Working plan is now [`docs/PLANNING_agentic_levers.md`](docs/PLANNING_agentic_levers.md) (levers on the *existing* pipeline). [`docs/PLANNING_ai_data_quality.md`](docs/PLANNING_ai_data_quality.md) is the retained **target/north-star** (+ the `review-by-exception` artifact). Concept ancestors `PLANNING_rework_feature.md` / `PLANNING_instance_flow.md` superseded. **Shipped:** L2b — wording step (`flow/item-flow-wording.ts`, agentic #934, unflagged); L3 — auto-approve gated on findings-clean (`isAutoApprovable`, agentic #935). **Next lever: L5** — per-finding review actions (choose-A/B, one-click fix/drop, verified-collapse, keep summary). Then L6 (supervisor emits findings), L7 (exemplar findings; needs `INTAKE_TO_SPEC` fix), L8 (standards learn from the existing loops). Harness: `npm run try:wording`.
  - **Review-by-exception delivery (plan §6b), in progress:**
    ...
    - ✅ **Session A wiring** — `FindingsJson` column on `agentic_runs`; findings computed in `item-flow` + persisted via result-handler + exposed on `getAgenticRun` (agentic #930). **Superseded for display by on-read (#933):** findings are now computed from the item's *current* content in `getAgenticStatus` (`computeReferenceFindings` → transient `CurrentFindings`), so manual edits show immediately and don't get wiped by a re-run; `FindingsJson` stays as a run-time snapshot. ⏳ intake-conflict findings still run-time-only on read.
    - ▶ **Session B** — ✅ shared `FindingsPanel`: "Zu prüfen" shown in the **KI tab** (persistent) + review wizard + per-field markers (ui #931/#932). ⏳ still: verified-collapse (hide unflagged fields), evidence from stored search sources, per-finding bounded-ask interactions (choose-A/B, one-click fix/drop). Note: findings only populate on runs completed after #957 (no backfill).
...
52. **Pre-existing failing tests (not caused by the #889–894 work; documented for cleanup).** `schema-contract-compatibility.test.ts` asserts the supervisor prompt contains `schema-contract.md`, but the current `supervisor.md` was rewritten to a description-quality focus and no longer references it — the test expectation is stale. `test/agentic-direct-dispatch.test.ts` and `test/agentic-startup-resume.test.ts` fail in the local esbuild harness because the scheduled `invokeModel` isn't observed (`toHaveBeenCalledTimes(1)` → 0); verify against CI's Postgres/jest setup. All three fail identically on a clean tree.
   ...
   - ✅ **`schema-contract-compatibility.test.ts` fixed** (was failing on a clean tree since before #925/#926): the test asserted `supervisor.md` contains the literal `schema-contract.md`, but the supervisor anchors to the canonical schema via the shared `{{OUTPUT_CONTRACT}}` + `{{TARGET_SCHEMA_FORMAT}}` tokens (same as extract.md), not the filename. Assertion updated to check those tokens.
...
- **Revive flag-based review metrics (deferred).** The review wizard makes one explicit approve/reject decision and, on reject, feeds a concrete before→after correction diff + notes into the next run (chosen over deriving flags — a diff is richer and unambiguous). Consequence: `bad_format` / `wrong_physical_dimensions` / `information_present` are sent at non-blocking defaults, so `AgenticReviewMetricsRows` (`% Schlecht formatiert / Falsche Dimensionen / Info unvollständig`) now reads all-positive. If those dashboards are still wanted, derive the flags **purely for metrics** (e.g. text edited ⇒ bad_format, dimension edited ⇒ wrong_physical_dimensions, required spec blanked ⇒ information_present=false), decoupled from the re-run path. Not needed for extraction quality.

...
0k. ✅ **Test suite stabilized post-Postgres migration, now re-hardened.** 420 tests passing; 9 skipped; 0 failing. 7 stale assertions fixed; `save-item.test.ts` rewritten (12 tests); `restart-review-metadata.test.ts` rewritten (3 tests); `dispatch-queue-concurrency.test.ts` rewritten (7 tests). ~41 test files still deferred in `jest.config.cjs testPathIgnorePatterns`.
0l. **Deferred test rewrites needed:** invoker-adapter + item-flow-* (old AgenticServiceDependencies interface), `test/csv-ingest-*.test.ts` + `test/export-items.test.ts` + `test/apiRoutes.test.ts` (SQLite db.exec/db.prepare), `test/frontend-agentic-review-flow.test.ts` (models alias resolution), `frontend/src/components/__tests__/PlacementScanView.test.tsx` (missing lib/logger mock path).

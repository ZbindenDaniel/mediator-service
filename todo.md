...
0z4. **AI runs optimization — phased implementation** (design: `docs/PLANNING_ai_runs_optimization.md`).
  ...
  - **Next — Phase 1b (Thread 2A):** surface stored search sources + per-run query count + reuse/live indicator in the KI tab (`LastSearchLinksJson` is persisted but never read by `frontend/src`). Then phase 2 (state-machine correctness: Thread 4A re-queue-vs-terminal + 4C empty-query repair), phase 3 (grounding: Thread 1), phase 4 (rework closure: Thread 3).

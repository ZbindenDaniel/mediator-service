...
0z1. ✅ **Agentic search-token burn: idle-fill re-ran the same queries every 5s.** The keep-busy dispatcher used a plain `SELECT` (`fetchIdleFillAgenticRuns`) that re-selected the same `notStarted`+`SearchQuery` runs (the default state for bulk-imported items) on every 5s dispatch tick, re-billing identical Tavily searches; it also never satisfied the `=== running` promotion guard, so the run never left `notStarted`. Fixed by replacing it with an atomic `claimIdleFillAgenticRuns` (`notStarted`→`running`, `FOR UPDATE SKIP LOCKED`) mirroring `claimQueuedAgenticRuns`, so each keep-busy run is claimed and dispatched exactly once. See agentic changelog #908.

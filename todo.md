...
50. **Rework mechanism / review-by-exception.** — Working plan is now [`docs/PLANNING_agentic_levers.md`](docs/PLANNING_agentic_levers.md) (levers on the *existing* pipeline). [`docs/PLANNING_ai_data_quality.md`](docs/PLANNING_ai_data_quality.md) is the retained **target/north-star** (+ the `review-by-exception` artifact). Concept ancestors `PLANNING_rework_feature.md` / `PLANNING_instance_flow.md` superseded. **Shipped: L2b** — dedicated wording step after extraction (`flow/item-flow-wording.ts`, `WORDING_STEP` flag, agentic #934). **Next levers:** L3 (auto-approve gated on wording/standards pass), L5 (per-finding actions), or harden extraction (the other half of L2).

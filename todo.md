...
46. ✅ **Spec contracts: contract version stamping.** `SpecContractVersion INTEGER` added to `agentic_runs` (additive), stamped on each completed run from `getSpecContract(sub).version`. Powers the idle contract-audit sweeper (#50 Phase 2a). See agentic changelog #890.

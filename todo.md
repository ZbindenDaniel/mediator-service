...
- **Multi-location deployment (in progress):** approved plan at `docs/PLANNING_multi_instance.md` — single cloud app + DB, per-location print agent + CUPS connected via a `/agent` WebSocket control plane, single shared `AGENT_TOKEN` (deliberate simplicity tradeoff for v1). Schema + `resolvePrinterQueue` per-site lookup + `/agent` endpoint shipped (see printing changelog 860). Still open: `backend/print-agent.ts` standalone entrypoint, admin "Worker nodes" UI (per-queue `Site`/`LabelTypes` editing, "Gather queues" button), `site` value in `frontend/src/lib/user.ts`, CUPS container port 631 + basic auth, `docker-compose.worker.yml`, retire `backend/utils/sync-printer-queues.ts`.

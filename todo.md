...
- **Shopware integration (P0 + P1 done; see `docs/PLANNING_shopware_integration.md`).** Direction decided:
  ...
  **#944 (variant re-sync 500 fixed):** `configuratorSettings` entries now carry a deterministic id (`sha1(parentId:optionId)`) so re-sync upserts instead of duplicating (`uniq.product_configurator_setting` 1062). The `mkdir Permission denied` on image upload is a dockware container fs-permission issue (env, not code); storefront axis grouping is a Shopware presentation setting.

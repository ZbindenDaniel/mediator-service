...
- **Multi-location deployment (in progress):** approved plan at `docs/PLANNING_multi_instance.md` — single cloud app + DB, per-location print agent + CUPS connected via a `/agent` WebSocket control plane, single shared `AGENT_TOKEN` (deliberate simplicity tradeoff for v1). Schema + `resolvePrinterQueue` per-site lookup + `/agent` endpoint shipped (see printing changelog 860). `backend/print-agent.ts` standalone entrypoint + AGENT_TOKEN claim-job/status HTTP API shipped (see printing changelog 861). `runPrintWorker()` in `server.ts` gated to untargeted jobs only, and `site` value in `frontend/src/lib/user.ts` shipped + wired through all print request call sites (see printing changelog 862). Admin "Worker nodes" UI shipped — agent `hello` auto-upserts discovered queues, `Site`/`LabelTypes` editing, "Queues abrufen" gather button (see printing changelog 863). `docker-compose.worker.yml` + `print-agent/Dockerfile` shipped, supports either the bundled CUPS container or an existing reachable CUPS server (e.g. the Pi setup) via `CUPS_HOST` (see printing changelog 865). Shared/cloud-side CUPS container's port 631 now exposed loopback-only (`127.0.0.1:631:631`, no basic auth — operator explicitly rejected basic auth due to prior headaches; see printing changelog 866 for why a wide-open bind would have been worse, not safer). nginx `/agent` WebSocket location added (auth_basic off, 3600s timeout), `requireAgentAuth` fail-closed fix, worker compose profiles (`--profile printing`) — see printing changelog 868. SSH tunnel setup for Ollama documented at `docs/detailed/ollama-tunnel-setup.md`. Still open: retire `backend/utils/sync-printer-queues.ts` — explicitly deferred until a worker/agent is actually deployed, since that script's `lpadmin` calls and the still-live `PrinterQueuesCard.tsx` admin CRUD depend on the current unauthenticated CUPS `/admin` access (decided 2026-06-30, see printing changelog 863 Deferred). Registry image publishing not set up — no CI/registry pipeline exists yet; worker stack builds locally for now (see printing changelog 865 Deferred). `docs/PLANNING_multi_instance.md` rewritten to describe the shipped architecture (was still describing the pre-implementation problem state); `cups/README.md` updated to note the dual deployment (co-located cloud fallback vs. per-location worker stack) — see printing changelog 867.

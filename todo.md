...
0z0a. ✅ **Intake items had no audit trail ("strange items" that appear only via exact-number search).** Intake creates the `items` instance directly in `intake-answer.ts` `ensureItem`, bypassing `add-item`/`import-item` (the only creators that call `logEvent`). Intake-born items therefore had an empty event log and, once their `Auf_Lager` reached 0, were hidden from every default list (list filters `Auf_Lager > 0`; search does not) — matching the reported "missing Artikelnummer 19503–19509, but search finds it, no logs" symptom. Fixed by emitting a `Created` event (Actor `intake-station`, `Meta` = source/artikelNummer/serial/mac) right after `persistItemInstance`, only on the create branch. See intake changelog #883.

**Discovered while investigating 0z0a — deferred (requester chose logging-only for now):**
- **Artikelnummer padding mismatch.** `intake-answer.ts:56` stores unpadded `"19503"`; `material-number.ts:21` (manual import) stores 6-digit `"019503"`. Same logical number, two string forms — all list/join/ERP comparisons are exact-string, so the populations diverge. Reconcile behind one shared allocator/formatter.
- **Non-atomic `MAX(...)+1` allocation.** `getMaxArtikelNummer` (`backend/db.ts:1523`) is read-then-write with no lock/reservation; concurrent intakes compute the same next number and the `item_refs` UPSERT silently merges them. Consider a reserved/locked allocation or a DB sequence.
- **`ensureItem` re-intake trap.** `ensureItem` keys on serial/MAC and returns the existing instance early; a `newRef` re-run allocates + creates a new ref but leaves the instance pointing at the old Artikelnummer → orphan ref + consumed number.
- **No way to see `Auf_Lager`=0/NULL items.** The list's `Auf_Lager > 0` gate (design decision, this file's Qty=0 note) hides such items with no operator-facing toggle to surface them.
- Intake still logs no event for ref creation or quality completion — only instance creation is covered.

...
0z4. 📝 **AI runs optimization — design doc for brainstorm.** `docs/PLANNING_ai_runs_optimization.md` analyses five agentic-run pain points as one design (grounding / search anchor, search-result visibility, rework closure, waiting→failed state machine, Ollama `UND_ERR_HEADERS_TIMEOUT`) with per-thread options + recommendations, a shared observability backbone, and a build sequence. **Next:** brainstorm the open questions, pick a phase-1 slice (proposed: observability backbone + Ollama dispatcher `headersTimeout`), then implement. No code changed yet.

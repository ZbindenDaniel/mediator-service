...
function makeCtx(itemExists = true) {
  const runMock = jest.fn();
  const prepareMock = jest.fn(() => ({ run: runMock }));
  return {
    getItem: { get: jest.fn(() => itemExists ? { ItemUUID: 'test-uuid' } : undefined) },
...
describe('edit-item-instance action', () => {
  ...
  describe('handle()', () => {
    ...
    it('includes UpdatedAt in the SQL SET clause', async () => {
      ...
      const sql: string = ctx._prepareMock.mock.calls[0][0];
      expect(sql).toContain("UpdatedAt=datetime('now')");
    });
  });
});

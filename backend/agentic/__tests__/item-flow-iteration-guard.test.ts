...
describe('runExtractionAttempts iteration schema key guard', () => {
  ...
  it('includes spec-contract field descriptions alongside missing_spec keys when available', async () => {
    const target = buildTarget();
    const extractionComplete = JSON.stringify(target);
    let capturedExtractionSystemPrompt = '';
    ...
    const result = await runExtractionAttempts({
      ...
      target,
      reviewNotes: null,
      missingSpecFields: ['RAM', 'Speicher'],
      missingSpecFieldDescriptions: {
        RAM: 'Memory in GB',
        Speicher: 'Storage size and type (e.g. 256 GB SSD)'
      },
      unneededSpecFields: [],
      skipSearch: false,
      transcriptWriter: null
    ...
    expect(result.success).toBe(true);
    expect(capturedExtractionSystemPrompt).toContain(
      'missing_spec: expected keys to include when evidence exists (RAM (Memory in GB), Speicher (Storage size and type (e.g. 256 GB SSD)))'
    );
  });

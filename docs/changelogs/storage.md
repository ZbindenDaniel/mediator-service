...
## 885. ✅ Inventory scan exit route: reconcile unscanned box items (remove stock or clear location)
   - **Why:** The scan-items-into-a-box flow (`PlacementScanView`, `BoxDetail` → "Einscannen") was add-only per `PLANNING_INVENTORY_ACTIONS.md` decision #1 — it moved scanned items *in* but never touched items recorded in the box that weren't scanned, so phantom stock (sold/relocated/lost items) lingered on the box forever. Added an **exit route**: on "Fertig" the view fetches the box's current instances, diffs them against the UUIDs confirmed present this session (moved in *or* already there), and surfaces the unscanned ones for resolution. Each offers two choices (per-item, with an "apply to all" shortcut): **Bestand entfernen** reuses the existing `POST /api/items/:uuid/remove` (`zeroItemStock` for Stk / decrement for Menge); **Standort entfernen** clears only the box assignment via a new `clearItemLocation` helper + `POST /api/items/:uuid/clear-location` action, keeping the item in stock. New `LocationCleared` event key. Chose to extend the existing placement flow rather than build the full `PLANNING_INVENTORY.md` verification feature (sessions table, MissingAt, passive triggers) — this is the focused change the request asked for and reuses existing endpoints/helpers.
   - **Deferred:** (1) `Menge` (bulk) items: "remove stock" decrements by 1 via the existing remove endpoint rather than zeroing a bulk quantity — v1 targets the common `Stk` instance case; a count-adjust UI is future work. (2) Not building `inventory_sessions`, `LastInventoryDate`, `MissingAt`, passive scan triggers, or a missing-items view (larger `PLANNING_INVENTORY.md` scope, still todo 19b). (3) No undo after resolution — a location-cleared item is re-placed by scanning it into a box again.

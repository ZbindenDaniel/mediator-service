...
## 933. ✅ Add WMS capability-landscape diagram (Mermaid) as an intern extension draft
**Why:** Requested a single editable picture of everything the WMS can do, to seed a full process landscape an intern will extend. Chose **Mermaid in a Markdown file** (`docs/diagrams/wms-capability-landscape.md`) over draw.io: it diffs cleanly in git, renders in Gitea/GitHub/VS Code, and is trivially editable by someone new — the existing `docs/overview.drawio` is an empty placeholder reserved for a later binary version. Two views: (1) a **capability map** of all nine live domains (intake, item lifecycle, agentic, storage, scanning, printing, media, ERP/shop, admin) laid out in three stacked bands (capture → create value → integrate) with the external systems (netboot, Tavily, Ollama, kivitendo, Shopware, CUPS, WebDAV, Postgres) attached; (2) an **end-to-end device-journey** process flow (arrive → intake scan → quality → agentic enrich → review gate → label → placement → ERP/shop). Domain boxes mirror the folder-README + topic-changelog map in `OVERVIEW.md`; planned/partial capabilities (transport boxes, inventory cycle, full auth) are dashed. Both diagrams validated by rendering to SVG/PNG via `@mermaid-js/mermaid-cli`. Bands are chained with invisible `~~~` links to force vertical stacking instead of one wide strip (noted inline as a WHY comment in the diagram source).
**Deferred:** Only the mediator-service's own capabilities are mapped — the surrounding operation (goods-in, dismantling workshop, sales beyond Shopware, returns, disposal/accounting) is left for the intern to add around the core, with in-file notes on how. No draw.io version produced; ownership annotations (who runs each step) left as an extension note.

...
## 935. ✅ nginx: raise `client_max_body_size` so backup/import uploads aren't rejected with 413
**Why:** `POST /api/import/validate` for a real backup (~4.7 MB) was killed by nginx's 1 MB `client_max_body_size` default before it ever reached the backend (`mediator-proxy … 413 … "client intended to send too large body"`). The proxy config never set the directive. Fixed with a single server-level `client_max_body_size 50m` in `config/nginx/mediator.conf` — a config-only change, applied to all locations including `/api/`. 50m gives headroom as backups grow; the backend enforces no body limit of its own, so nginx was the only gate.
**Deferred:** Nothing deferred. Did not add a tighter per-location cap or a separate limit for non-import routes — a single generous server-level value is simplest and the routes are already behind basic-auth + rate limiting.

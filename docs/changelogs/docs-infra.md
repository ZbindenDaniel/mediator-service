...
## 900. ✅ License the project under AGPL-3.0-or-later
   - **Why:** The repo had no `LICENSE` file and `package.json` carried no `license` field, so the code was implicitly all-rights-reserved — a blocker for open-sourcing. Owner intent is to open-source *and* have downstream improvements/modifications flow back, including from anyone who runs it as a hosted service. A license-inventory review (`docs/license-inventory.json`, 437 deps) confirmed the entire dependency tree is **permissive** (MIT/ISC/BSD/Apache-2.0/0BSD, no GPL/LGPL/AGPL/MPL copyleft), so there was **no compatibility constraint** forcing any particular choice — the decision was purely about intent. Chose **AGPL-3.0-or-later** over MIT/Apache precisely because this is a network-facing "mediator-service": AGPL §13 (Remote Network Interaction) is the only common license that closes the SaaS loophole, requiring operators of a modified hosted version to offer users its source. Wrote the **verbatim** FSF license text to `LICENSE` (661 lines, §13 present — sourced as an exact copy, not a summary), set `package.json` `"license": "AGPL-3.0-or-later"` (SPDX; `-or-later` per the FSF boilerplate's "or any later version"), and added a `## License` section to `README.md` with the copyright line (revamp-it, 2026), the AGPL notice, an explicit §13 network-use callout, and a pointer to `THIRD_PARTY_NOTICES.md`.
   - **Deferred:** (1) **Per-file source headers** — the FSF recommends a boilerplate notice atop each source file; not applied across the existing tree (large, mechanical) — the root `LICENSE` + README notice are sufficient to license the work. (2) **In-app "Source" link** — AGPL §13 is best satisfied by the running service exposing a link to its corresponding source; no UI affordance was added, so operators currently satisfy §13 by other means (e.g. the public repo). (3) **`private: true`** left in `package.json` — it only guards against accidental `npm publish` of this non-package service and does not affect the source license or open-sourcing on a git host. (4) Two `UNKNOWN`-licensed deps flagged in `THIRD_PARTY_NOTICES.md` (`png-js`, `exit`) still want manual license confirmation before any binary redistribution; both are MIT upstream. (5) Did not add a `CONTRIBUTING.md`/CLA — inbound=outbound (AGPL) is assumed.

...
## 899. ✅ New-use-case readiness inventory + spare-part scoping (`docs/PLANNING_NEW_USE_CASE.md`)
   - **Why:** A new business use case is coming, framed as "change the config, categories, and contracts." Produced a discovery/readiness inventory of every touchpoint coupled to the current IT‑refurbishment use case, then — once the use case and deployment model were confirmed — extended it into a feature-disposition plan. Findings: **contracts** are the ready layer (runtime JSON, auto‑discovered by `<subcode>.json`); **categories** are the coupling hotspot — the taxonomy lives in **four** hand‑synced representations (`models/item-categories.ts`, `frontend/src/data/itemCategories.ts` re‑export, `docs/data_struct.md` LLM reference copied to dist by `scripts/build.js`, and the hardcoded `INTAKE_CATEGORIES`) with no generator + ~16 consumers; **config** has no use‑case/domain/tenant dimension. Decisions captured: (1) the use case is **thorough spare-part cataloging** (catalogue every reusable part) — the existing disassembly/component lifecycle is the centerpiece to strengthen, not build; (2) it runs as a **separate deployment** that must itself be **multi-tenant**. Two greenfield cross-cutting gaps confirmed by grep: **no tenant/mandant/org_id concept exists anywhere**, and **no unified feature-flag/capability system** (toggles are ad-hoc `*_ENABLED` env vars + a client-side "simple mode" CSS class — nothing disables a backend feature *and* hides its UI). Feature-by-feature disposition (keep / strengthen / remove-by-config / out) recorded with a recommended sequencing. No code changed; documentation only. Registered in `docs/PLANS.md`; tracked as todo #53.
   - **Deferred:** Implementation. Domain input still needed: the concrete spare-part taxonomy + per-part quality/spec contracts. Two open product decisions: D1 keep-vs-drop scanning/QR (recommend keep), D2 drop-vs-repurpose CO₂ scoring (recommend drop). The feature-flag system (§10.2) and multi-tenancy (§10.1) each warrant their own phased plan. Also still open: the taxonomy→`data_struct.md`/intake generator (G‑C1) and the `contracts/README.md` `disassembly/` vs on‑disk `assembly/` doc drift (G‑K3).
   - **Follow-up (§12 architecture proposals):** Added high-concept designs for the two greenfield workstreams, grounded in the real leverage points. **Feature flags:** a single capability manifest driving *both* a backend hard-gate at the one dispatch chokepoint (`server.ts:960`, via an optional `feature?` tag on the `Action` interface) *and* frontend soft-hide (a served `GET /api/app-config` — none exists today — consumed by a `useFeature` hook, reusing the existing `simpleMode` CSS body-class pattern). Coarse feature-group granularity (1 flag per subsystem), superseding the scattered `*_ENABLED` vars + `simpleMode`; backend = security boundary, frontend = UX only, so they can't drift.
   - **Follow-up (§12.3 tenancy refined to the hosting model):** The hosting model is *we host for multiple external orgs who contribute stock; the payoff is a shared spare-part catalogue all orgs can use, but logistics stays private per tenant.* This is **not** flat "shared reads" — it is a **two-tier visibility model** that maps onto the schema's existing **reference↔instance seam**: `item_refs` (part descriptions/specs, keyed by `Artikel_Nummer`) stay the **shared catalogue with no `TenantId`** (read by all; writes guarded by an optional `ContributedByTenant`), while `items`/`boxes`/shelves/`box_stubs`/per-instance quality/logistics events carry **`TenantId` and are hard-isolated** (reads *and* writes filtered by `ctx.tenant`, no shared fallback). Tenants + groups (normal / super / platform-admin via a designated admin tenant) are managed **in Authentik**; the app stores a tenant reference and resolves tenant+group once at the chokepoint from forward-auth headers. Super users create shelves for their tenant; nobody sees another warehouse. Class-aware scoping enforced in the `db.ts` layer (ref-level vs instance-level functions already separate). Additive nullable migration on logistics tables only; `item_refs` untouched. Three design decisions since **resolved**: **DL1** — `BoxID`/shelf IDs are not autoincrement but structured, globally-unique minted TEXT keys from a global counter (boxes/shelves also already carry a human `Label`), so they don't collide across tenants; keep the scheme and add `TenantId` as a *visibility* column, not part of the key (global uniqueness is a feature — a scan maps to one row, tenant isolation then gates visibility). Real follow-on: shelf IDs encode a location and `shelfLocations` is a global hardcoded list → **per-tenant shelf locations** (folds into §6 config externalization); optional cosmetic tenant-prefix on minted box IDs. **DL2** — not a marketplace (a separate shop may come later); catalogue exposes a **global aggregate quantity** (controlled cross-tenant `SUM GROUP BY Artikel_Nummer`, total only) while locations stay private — item detail = shared ref + global count + own-tenant instances. **DL3** — self-resolving given globally-unique IDs. Also noted: **self-registration via org token** is an Authentik enrollment concern (registration → group assignment) with no data-model impact — the app still reads tenant/group from forward-auth headers. Per-tenant taxonomy/DBs deliberately out of scope (shared catalogue implies one shared taxonomy).

...
## 957. ✅ Planning reset to a levers doc (`PLANNING_agentic_levers.md`)
**Why:** The AI-data-quality planning had drifted into a from-scratch redesign — six pillars, a new Finding/Assessment object model, a Workbench page, an instance/reconcile flow with its own prompt. For a live pipeline that's unbuildable as one step, and the operator (correctly) flagged that the goal was always to **find levers to move the existing system**, not to redesign it. Reframed without discarding the thinking: `PLANNING_ai_data_quality.md` and the `review-by-exception` artifact are retained as the **target/north-star** (the *why* + finished shape), and a new `PLANNING_agentic_levers.md` becomes the **only working plan**, held to one rule — every lever must point at a file that ships today. It inventories the latent machinery (extraction/`extract.md`, supervisor/`supervisor.md` + `{{SUPERVISOR_REVIEW}}` slot, guidance injection, the findings engine, `ambiguousFields`, the `AUTO_APPROVE`/`AUTO_REWORK` flags, the reject-feedback loop, `agentic_run_snapshots`) and lists levers L1–L9 (scope tag on findings; standards→prompt prevention; auto-approve on zero findings; legible sweeper; per-finding actions + verified-collapse; supervisor emits findings; exemplar findings on read; reject-feedback→standards candidate; run provenance) ordered by leverage, with a lean build order. The redesign-shaped pieces (Workbench, quality-grade formula, full reconcile flow, LLM standards-auditor) are explicitly **parked** as emergent target-state, reached by accumulating levers, not built up front. Added a TARGET banner to the top of `PLANNING_ai_data_quality.md`.
**Deferred:** Nothing built — this is a planning-frame reset. L1 (scope on the finding object) is the first concrete lever when we resume code.

## 956. ✅ Unified plan: **AI Data Quality Lifecycle** (`PLANNING_ai_data_quality.md`)
**Why:** Three overlapping planning docs (rework-feature concept, instance-flow/reconciliation, ai-runs-optimization) plus the operator's five real pain points (no review capacity; approved items decay as quality evolves; reviews hit the same issues repeatedly; intake and review are split across time+people; no broad data overview) were circling the *same* goal from different angles. Consolidated into one plan anchored on the pain, not on any mechanism: reframed the whole thing as a **data-quality lifecycle** (produce → grade → surface → triage → review/rework → re-grade as standards evolve) with six pillars — A the KI-Runs/data-quality **Workbench** (one surface), B a **quality grade** (completeness/conformance/freshness), C **Standards separated from schema** (runtime-editable house rules that both steer generation and grade/audit existing items — the systematic fix for "same issues every review"), D **rework as the single execution arm** (folds levers L1–L5), E **reconciliation** as one evidence source (demoted from centerpiece), F **auto-approve** as the throughput valve. Also diagnosed the spec-contract **"smell"** the operator flagged: the contract legitimately defines "complete," but one JSON file wears six hats (schema/guidance/completeness/approval/staleness/shop-filter) with no change-governance, and the recent capability-restructure changed field *meaning* without updating consumers (`INTAKE_TO_SPEC` orphaned; auto-approve+sweeper grading against the wrong fields) → Phase 0 un-overloads it (schema vs standards vs policy). Phasing is surface-before-automation. Superseded `PLANNING_rework_feature.md` and `PLANNING_instance_flow.md` with banners (instance-flow retained as detailed reference for the reconciliation-object schema until it migrates); repointed todo #50; merged origin/main first (instance-flow doc + shipped agentic work).
**Deferred:** Everything actionable — nothing scheduled, no code, no migration. Open questions on grade formula, standards representation, re-grade trigger, and the auto-approve/grade relationship carried into the plan.

## 955. ✅ Rework-feature concept doc (`PLANNING_rework_feature.md`) — *superseded by #956*
**Why:** Rework works mechanically but reads as several half-connected mechanisms plus an idle sweeper that fires invisibly — the operator won't enable `AUTO_REWORK` because it's unclear what it does or where it starts from. The rework story was scattered across `PLANNING_ai_runs_optimization.md` (Thread 3, Phase 4), todo #50, and code with no single conceptual home. Wrote a dedicated **concept-only** doc: entry-point inventory (5 ways a run (re)starts), the four reasons it feels like a shadow (no run provenance; invisible+indirect sweeper trigger with no event/dry-run; in-memory `pendingRework` map lost on restart; rework can demote an approved item to `failed`), a unifying model (rework = regenerate a field subset, preserve the rest, record *why*, never destroy prior good state), and a lever menu (L1 provenance / L2 legible sweeper / L3 persist pending rework / L4 failure closure / L5 reachable-from-review) with tradeoffs. Cross-linked from todo #50. Deliberately no scheduling, no code, no migration — the point is to make the option space legible before deciding.
**Deferred:** Everything actionable — no lever is scheduled or built. Auto-approve integration noted as common ground but explicitly out of scope. **Now folded into #956.**

## ✅ Intake image / contract docs: sync surface + stale disassembly→assembly fixes
   - **Why:** Documentation follow-up to the spec/accessory intake work (intake changelog #900) and the deferred-identity components (#899). Added the **contract → script sync** surface to `docs/detailed/intake-image-guide.md` (what `qualityQuestions` the script receives, which contract edits need no script change vs. which do, answer value formats, and what deliberately does not cross the wire); added runnable `GET /api/contracts/{specs|quality|assembly}/…` requests to `intake-image.http`; and noted the merged assembly questions + scan-derived spec back-fill in the `intake.md` runbook. Also fixed **stale `disassembly` references** left over from the `disassembly/`→`assembly/` rename (#850): `contracts/README.md`, `backend/lib/README.md`, `docs/detailed/README.md`, `contracts/quality/README.md`, and a full refresh of `docs/detailed/spare-parts-catalog.md` (assembly schema `question`/`specQuestion`/`noLink`/`multipleAllowed`, `assemblyToQualityContract`, `/api/contracts/assembly/…`, contract-gated parent Ersatzteil, and remove-from-device doubling as graduation).
   - **Deferred:** Consolidating the slot key onto `item_relations."SlotKey"` — intake-created components write it, but `catalog-spare-part` and the spare-parts reads still use `Notes` (noted as an open question in spare-parts-catalog.md). Docs-only change; no code touched.

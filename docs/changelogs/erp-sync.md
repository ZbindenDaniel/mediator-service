...
## 936. ✅ Event-log export/import round-trip no longer corrupts `Meta` into `[object Object]`
...
Changes:
- **`backend/actions/export-data.ts` (root cause):** `toCsvValue` now `JSON.stringify`s object/array values (jsonb columns like `Meta`) instead of `String()`-coercing them, so the CSV carries valid JSON that round-trips. `Date` is excluded so timestamp cells keep their existing formatting. This is column-agnostic — any future jsonb/object cell serializes correctly, not just `Meta`.
- **`backend/importer.ts` (defense + repairs already-broken exports):** new `sanitizeEventMetaValue` validates the `Meta` cell with `JSON.parse` before insert; a non-JSON value (a legacy `[object Object]` artifact from an export made before this fix) is dropped to `NULL` with a warning so the **event still imports** rather than being lost. Mirrors the guarded legacy-`Meta`→`NULL` sanitization already done in `initDb` (docs-infra #878).
- **Test:** `test/csv-ingest-events-meta.test.ts` — valid JSON Meta passes through unchanged; `[object Object]` imports as a row with `Meta=NULL` (count 1, skipped 0); empty Meta → NULL.

**Deferred:** No data migration to repair `Meta` values already stored as strings in the live DB (none observed — the corruption only occurred in the CSV boundary, and rows that hit it were dropped, not persisted). Old backup ZIPs made before this fix still lose the `Meta` payload of affected rows on restore (the object was never recoverable from `[object Object]`); those events now import with `Meta=NULL` instead of being skipped entirely.

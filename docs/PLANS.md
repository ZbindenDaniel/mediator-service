...
- [x] **UI grouping** – update UI list/detail grouping to surface instance `1` as the primary record with minimal UI changes; confirm user-facing labels stay consistent. Done: `frontend/src/lib/itemGrouping.ts` drives grouping and is consumed by `frontend/src/components/ItemList.tsx` + `frontend/src/components/BoxDetail.tsx`. Reason: keep operator workflows stable while collapsing duplicates. TODO(ui-grouping-doc): keep this checklist aligned with grouped UI usage.

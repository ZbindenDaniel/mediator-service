...
| Path | Purpose |
| --- | --- |
| `backend/agentic/config.ts` | Loads model, search, callback, and Shopware settings from environment variables with zod validation so the orchestrator shares mediator config patterns.【F:backend/agentic/config.ts†L1-L86】 |
...
| `backend/agentic/utils/` | Hosts the shared external callback helper, rate limiter, JSON utilities, and LangChain adapters now consumed by the flow.【F:backend/agentic/utils/external-api.ts†L1-L87】 |

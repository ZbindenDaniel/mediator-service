...
| `WORDING_STEP` | `false` | When true, a dedicated LLM pass runs **after extraction** and rewrites the prose fields (`Artikelbeschreibung`, `Kurzbeschreibung`) into the house style, stripping fluff/marketing/source-copied phrases per `contracts/standards.json` — without changing facts. Lets extraction stay fact-oriented and centralizes wording. Skipped in rework mode; a failure or timeout keeps extraction's wording (never blocks the run). Opt-in because it rewrites customer-facing copy. |

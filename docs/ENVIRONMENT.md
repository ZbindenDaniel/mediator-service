...
| `MODEL_NUM_CTX` | `16384` | Ollama context window (`num_ctx`, tokens). Ollama's own default (~2-4k) is far below the extraction prompt (~6-7k tokens), so the prompt is silently left-truncated and the model returns an empty completion (the `json match missing` / `EXTRACTION_FAILED` loop). Default 16384 leaves room for the deployed gpt-oss:20b **thinking** model's reasoning pass (2-4k) + the JSON answer on top of the prompt; the model supports 131072, so raise if VRAM allows (KV cache grows with num_ctx), lower on a smaller GPU. Ollama provider only. |
| `MODEL_FORMAT_JSON` | (off) | Force Ollama responses to valid JSON (`format: "json"`). Strong guard against empty/prose output, but fights a thinking model's reasoning pass — **leave off for gpt-oss / any reasoning model**. Truthy = `1`/`true`/`yes`/`on`. |

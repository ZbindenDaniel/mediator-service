...
## UI
- **MVP** — Minimal `/chat` page using the shared `card` class keeps the conversation in memory and renders the agent's proposed SQLite queries for the `item` schema.
...
## Backend
- **MVP** — HTTP action accepts chat messages, spins up `chatFlow`, and returns the agent's response payload (including proposed queries); include structured logging and guarded error handling.

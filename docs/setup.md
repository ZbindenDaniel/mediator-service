...
6. Media storage defaults to local for development (`MEDIA_STORAGE_MODE=local`), using the backend `media/` directory unless `MEDIA_DIR_OVERRIDE` (or `MEDIA_DIR`) is set; container deployments can point to WebDAV by setting `MEDIA_STORAGE_MODE=webdav` and `WEB_DAV_DIR` to the mounted path.

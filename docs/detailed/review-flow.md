...
- Key fields (review-related):
  ...
  - Open: the structured flags (`bad_format`, `wrong_physical_dimensions`, `information_present`) are currently sent at non-blocking defaults (the wizard no longer collects per-step pass/reject); deriving them from wizard inputs is a pending decision (see `todo.md`).

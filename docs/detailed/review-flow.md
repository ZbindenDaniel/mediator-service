...
- Key fields (review-related):
  ...
  - Review wizard (`AgenticReviewWizard`): the manual review is a single wizard — Artikelbeschreibung → Kurztext → Spezifikationen → Dimensionen → Preis (Zurück/Weiter, edit-in-place + per-step feedback note) → a **summary** step with Ablehnen/Freigeben → Shop step (only after Freigeben). The decision is explicit: the payload carries `decision: 'approved'|'rejected'`, which the backend honors over checklist-derived signals. Per-step notes are tagged and concatenated into `LastReviewNotes`/`notes`.
  - Reject feedback loop: on **reject**, the wizard folds a concrete before→after correction diff (changed texts/dimensions/price, removed or changed specs) plus the notes into `LastReviewNotes`, which already flows into the next run's extraction/categorizer/supervisor prompts (`reviewNotes` → `sanitizedReviewerNotes`) and survives restart. On **approve**, edits are persisted to `item_refs`, so the corrected values are the next run's baseline — no diff is emitted. This replaces per-step flag derivation.
  - Open: the structured flags (`bad_format`, `wrong_physical_dimensions`, `information_present`) are sent at non-blocking defaults; the explicit `decision` drives approve/reject. Flag-based review **metrics** (`AgenticReviewMetricsRows`) therefore read as all-positive — reviving them is deferred (see `todo.md`).

...
## Core concepts
- CSV import staging, parsing, and deterministic archive ingestion order.
- Export contract variants (`manual_import`, `automatic_import`) and field ordering.
- ERP sync handoff boundaries and profile/mapping requirements.
- Partial-failure reporting strategy across multi-stage jobs.
...
- `backend/actions/csv-import.ts`
- `backend/actions/export-items.ts`
- `backend/actions/sync-erp.ts`
- `backend/importer.ts`
- `docs/erp-sync.sh`

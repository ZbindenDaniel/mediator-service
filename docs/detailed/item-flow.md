...
- When a skip **is** honoured, the stored sources are still injected as the extraction's search context — a skip suppresses *new* searches, it does not withhold the *collected* results. `item-flow-search.ts` pushes one `SearchContext` whose `text` is the stored sources formatted via `formatSourcesForRetry` (numbered `Title`/`URL`/`Description` blocks, `Description: (none)` when a source has no snippet), so a description-less persisted source (`{url,title?,description?}` with no description) still contributes its title + URL rather than collapsing to an empty context. Extraction labels this block `Previously collected search results (search skipped — extract from these; do not assume no evidence):` (vs. the normal `Current search context:`).
- Operators can treat no-search as **fully enforced** (both the initial gate and follow-up expansion), with an automatic live-search fallback when there is nothing stored to reuse — and the reused evidence is always presented to the model, never replaced by a bare "search skipped" note.

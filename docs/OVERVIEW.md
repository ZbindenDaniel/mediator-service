...
- **Agentic reviewed-example migration fallback**: hardened invoker example selection to log and fallback to `ReviewState` when `LastReviewDecision` is unavailable during one-time productive DB migration, preventing runtime query failures while keeping behavior scoped.

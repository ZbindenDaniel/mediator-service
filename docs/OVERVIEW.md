...
## Progress Updates
- **Agentic trigger duplicate-start guard + stable response status/message contract**: added a pre-start active-run check in `backend/actions/agentic-trigger.ts` that ignores duplicate queued/running starts with explicit `status`+`message`+`reason` payloads, logs duplicate/state-conflict/unexpected start failures with sanitized context, and keeps trigger error details compatible for frontend reason extraction.

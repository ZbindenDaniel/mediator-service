...
## Progress Updates
- Refined the QR scan button to sit inside search inputs (including add-item) with the Suchen action on its own line, aligned relocation scan controls next to the location selector, and routed QR-style searches directly to item/box destinations without calling search APIs.

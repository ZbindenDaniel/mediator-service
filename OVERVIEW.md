...
936. ✅ Phased plan to externalize the category taxonomy (`docs/PLANNING_TAXONOMY_EXTERNALIZATION.md`): single-source `taxonomy.json` → build-time codegen of the TS data module + `data_struct.md` + intake validation, chosen because `models/item-categories.ts` compiles into both backend and browser (rules out runtime file-load); preserves the module's public API so ~16 consumers are untouched; closes the 4-copy drift gap G-C1 → [docs-infra]

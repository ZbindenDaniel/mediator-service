...
900. ✅ New-use-case readiness inventory: catalogued every touchpoint coupled to the current use case (contracts = ready/drop-in JSON; categories = 4 hand-synced copies + ~16 consumers; config = no use-case dimension exists) as `docs/PLANNING_NEW_USE_CASE.md`, with gaps and the pivotal coexist-vs-replace decision → [docs-infra]

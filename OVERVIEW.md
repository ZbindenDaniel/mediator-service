...
917. ✅ Planning: **instance agentic flow** — one run engine typed by `(scope, mode)`; a per-`ItemUUID` flow (scope=instance) consumes intake data (`IntakeScan`, `InstanceSpecs`, quality answers, Phase-2 files) to fill instance-scoped specs and **reconcile** the device against its reference (`match`/`missing_on_ref`/`wrong_ref`/`instance_variance`). It writes only `InstanceSpecs`; its **only** route to change a ref is to *trigger a `rework` run* through the existing auto-approve gate (never mutates `item_refs`; data stays on the item). Reconcile is a final **non-blocking stage** after supervisor emitting a run-level data-quality metric + device↔ref verdicts. Introduces `scope`/`measuredSignal` on the spec contract (replaces the hardcoded `INTAKE_TO_SPEC` map), run **history** via a latest-row snapshot + append-only `agentic_run_history` (existing readers untouched), intake `/complete` as first trigger, and a new **`KI-Runs` list type** (reusing the item/box/activity list+detail+tab shell). Design agreed; phased in `docs/PLANNING_instance_flow.md` → [agentic]

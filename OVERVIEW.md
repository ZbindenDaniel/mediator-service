...
933. ✅ Added a WMS capability-landscape diagram (Mermaid) at `docs/diagrams/wms-capability-landscape.md` — a capability map of all nine live domains (three bands: capture → create value → integrate, external systems attached) plus an end-to-end device-journey process flow, as an editable draft for an intern to extend into a full process landscape. → [docs-infra]

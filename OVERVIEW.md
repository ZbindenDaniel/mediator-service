...
917. ✅ Planning: **instance agentic flow** — a per-`ItemUUID` flow that consumes intake data (`IntakeScan`, `InstanceSpecs`, quality answers, Phase-2 files) to fill instance-scoped specs and **reconcile** a physical device against its reference, surfacing `wrong_ref`/`missing_on_ref` insights with propose-only operator actions (never writes the ref). Introduces `scope`/`measuredSignal` on the spec contract (replacing the hardcoded `INTAKE_TO_SPEC` map), a run-identity change (instance-keyed `agentic_runs`), an auto-approve hard gate on bad-ref verdicts, and intake `/complete` as the first auto-trigger. Design agreed; phased in `docs/PLANNING_instance_flow.md` → [agentic]

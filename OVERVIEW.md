...
911. ✅ Fix intake reference matching + "HP HP HP" brand triplication: `/api/intake/start` now searches the scanned model against `Artikelbeschreibung`/`Suchbegriff`/`Kurzbeschreibung` (imported refs keep the model in `Artikelbeschreibung`, not `Kurzbeschreibung`) and strips the duplicated vendor the netboot image embeds in `model` before matching; the new-ref name builder strips a leading brand so `Hersteller` is prepended exactly once (new `backend/lib/intake-naming.ts`) → [intake]

...
960. ✅ **Manual ERP sync now marks `LastSyncedAt` on unpadded-Artikelnummer refs**: the handler fed `markRefsSynced` the zero-padded media-folder scope (`001158`) instead of the raw stored key (`1158`), so `UPDATE item_refs … WHERE "Artikel_Nummer" IN (…)` matched nothing for refs stored unpadded — they never got a sync timestamp (still HTTP 200, fire-and-forget mark). New `resolveSyncedArtikelNummern` returns the raw as-stored set; media scope keeps padding for folder names. Nightly path was already correct. → [erp-sync]
